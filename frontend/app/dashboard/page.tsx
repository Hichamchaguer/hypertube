  ...
  React.useEffect(() => {
    const getUser = async () => {
      try {
        ...
        if (!response || typeof response.user === "string") {
          router.push("/signin");
          return;
        }

        setUserName(response.firstName || response.username || "there");
      } catch (error) {
    ...
    <div className="space-y-12 pb-12">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-12 md:px-20 max-w-3xl space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-primary text-[10px] font-black uppercase tracking-[0.2em] rounded-md text-white">
                  Featured Movie
                </span>
            ...
            <div className="flex items-center gap-4 pt-4">
              <Link href={`/movie/${featuredMovie.id}`}>
                <Button
                  size="lg"
                  className="h-14 px-10 gap-3 rounded-2xl bg-white text-black hover:bg-white/90 font-bold border-none shadow-xl shadow-white/5"
                >
              ...
              <Button
                size="lg"
                variant="outline"
                className="h-14 px-10 gap-3 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold border-white/10 backdrop-blur-md"
              >
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted/80 font-medium">
          Ready to continue your movie journey?
        </p>
      </div>
